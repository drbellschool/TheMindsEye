...
export function HistoricalImageUploadQueue({ manager }: { manager: HistoricalImageUploadManager }) {
  if (manager.entries.length === 0) return null;
  const renderEntry = (entry: HistoricalImageUploadManagerEntry) => {
    ...
    const isActive = !["complete", "canceled"].includes(entry.progress.phase);
  ...
  if (!manager.expanded) return <button className="historical-image-upload-queue historical-image-upload-queue--collapsed" onClick={manager.expand} type="button" aria-label="Expand uploads">Uploads {manager.aggregate.active} active · {manager.aggregate.percent}%</button>;
  return <aside className="historical-image-upload-queue" aria-label="Historical image upload progress" aria-live="polite"><header><strong>Historical image uploads</strong><span>{manager.aggregate.completed} of {manager.entries.length} complete · {formatBytes(manager.aggregate.uploadedBytes)} of {formatBytes(manager.aggregate.totalBytes)} · {manager.aggregate.percent}%</span><button className="sanborn-button" onClick={manager.collapse} type="button">Collapse</button></header><div className="historical-image-upload-queue__overall"><span style={{ width: `${manager.aggregate.percent}%` }} /></div>{manager.entries.map(renderEntry)}<footer><button className="sanborn-button" onClick={manager.collapse} type="button">Collapse</button><button className="sanborn-button" onClick={manager.clearCompleted} type="button">Close completed</button></footer></aside>;
}
