...
const maximumManualGpsZoom = 22;
const maximumFitGpsZoom = 21;
...
function getMapPiecePlacementLabel(placement: SanbornMapPieceGeoreference | null | undefined): string {
  switch (deriveCanonicalMapPiecePlacementStatus({ placement })) {
    case "reviewed": return "Reviewed placement";
    case "placed": return "Saved placement";
...
function getMapPiecePlacementClass(placement: SanbornMapPieceGeoreference | null | undefined): string {
  const status = deriveCanonicalMapPiecePlacementStatus({ placement });
  return status === "reviewed" ? "is-reviewed" : status === "placed" ? "is-saved" : status === "draft" ? "is-draft" : status === "unable_to_place" ? "is-unable" : "is-unplaced";
}
...
function getGpsTownZoomFromState(studioState: HistoricalMapStudioState): number {
  ...
  return Math.min(maximumFitGpsZoom, Math.max(minimumAutoGpsZoom, preferredZoom));
}
...
export function HistoricalMapStudio({
  ...
  const [activeEditionHydrationState, setActiveEditionHydrationState] = useState<"loading" | "ready" | "error">("loading");
  const [bootstrapError, setBootstrapError] = useState("");
  const [bootstrapRetryNonce, setBootstrapRetryNonce] = useState(0);
  const bootstrapEditionRef = useRef("");
  ...
  const requestExternalMapView = useCallback((center: GeoCoordinate, zoom: number, source: MapViewChangeSource, path: string) => {
    ...
    setMapCenter(center);
    setModernMapZoom(Math.min(maximumManualGpsZoom, Math.max(0, zoom)));
  ...
  useEffect(() => {
    const townPackageId = initialData.activeTownPackage?.id;
    const mapYear = initialData.activeMapYear;
    if (!townPackageId || mapYear == null) {
      setActiveEditionHydrationState("ready");
      return;
    }
    const editionKey = `${townPackageId}:${mapYear}:${initialData.atlasInventory.activeAtlasId ?? ""}`;
    if (bootstrapEditionRef.current === editionKey && bootstrapRetryNonce === 0) return;
    bootstrapEditionRef.current = editionKey;
    bootstrapAbortRef.current?.abort();
    const controller = new AbortController();
    bootstrapAbortRef.current = controller;
    setActiveEditionHydrationState("loading");
    setBootstrapError("");
    const params = new URLSearchParams({ townPackageId, mapYear: String(mapYear) });
    if (initialData.atlasInventory.activeAtlasId) params.set("atlasId", initialData.atlasInventory.activeAtlasId);
    fetch(`/api/community/historical-map-studio/bootstrap?${params.toString()}`, { cache: "no-store", signal: controller.signal })
      .then(async (response) => {
        const payload = await response.json().catch(() => null) as { ok?: boolean; message?: string; state?: HistoricalMapStudioState } | null;
        if (!response.ok || !payload?.ok || !payload.state) throw new Error(payload?.message ?? "Saved reconstruction work could not be loaded.");
        return payload.state;
      })
      .then((state) => {
        if (controller.signal.aborted || state.activeTownPackage?.id !== townPackageId || state.activeMapYear !== mapYear) return;
        const nextSelectedAssetId = state.sheets.some((sheet) => sheet.assetId === selectedAssetId) ? selectedAssetId : state.sheets[0]?.assetId ?? "";
        setSheets(state.sheets);
        setAtlasInventory(state.atlasInventory);
        setTownIndexRegions(state.townIndexRegions);
        setBirdsEye(state.birdsEye);
        setGeoHistory(buildInitialSheetGeographicHistory(createSheetGeographicPresentFromState(state)));
        setHistory(buildInitialHistory(createPresentFromState(state)));
        setGeoreferenceDraft(createGeoreferenceDraft(state, nextSelectedAssetId || null));
        setSelectedAssetId(nextSelectedAssetId);
        setMapPieceGeoreferences(mergeSavedAndDefaultMapPieceGeoreferences(state.atlasInventory.pieces, state.mapPieceGeoreferences));
        setLastSavedAt(state.workspace?.updatedAt ?? "");
        setResolvedMapPlacementWorkspaceId(state.workspace?.workspaceId ?? "");
        setBootstrapError("");
        setActiveEditionHydrationState("ready");
      })
      .catch((error) => {
        if (controller.signal.aborted) return;
        setBootstrapError(error instanceof Error ? error.message : "Saved reconstruction work could not be loaded.");
        setActiveEditionHydrationState("error");
      });
    return () => controller.abort();
  }, [bootstrapRetryNonce, initialData.activeMapYear, initialData.activeTownPackage?.id, initialData.atlasInventory.activeAtlasId]);
  ...
  async function saveActiveWorkflow() {
    switch (atlasWorkflowStep) {
      ...
      case "gps_alignment":
        if (!selectedMapPiece) {
          setSaveStatus("error");
          setSaveMessage("Select a geographic object before saving placement.");
          return;
        }
        if (!selectedMapPieceHasGeographicFootprint && selectedMapPieceGeoreference?.placementStatus !== "unable_to_place") {
          setSaveStatus("error");
          setSaveMessage("Click the modern map to place this object before saving.");
          return;
        }
        return saveSelectedMapPiecePlacement();
    }
  }
  ...
  function openPlacementQueueItem(item: MapPiecePlacementQueueItem) {
    ...
    if (item.status === "not_placed") {
      setPiecePlacementAnchorId(item.pieceId);
      setPlacementAnchorAssetId("");
      setGeoEditMode("pan_modern_map");
      commitGeographicMapSettings({ editMode: "pan_modern_map", globalHistoricalOpacity: 1 }, false);
      setSaveStatus("idle");
    ...
    // Selecting another object must not overwrite the worker's current map
    // center or zoom. Fitting remains an explicit Alignment action.
  }
  ...
  function movePlacementQueue(direction: "previous_unplaced" | "next_unplaced" | "next_review") {
    ...
    const matches = direction === "next_review" ? items.filter((item) => item.status === "placed") : items.filter((item) => item.status === "not_placed");
  ...
  function resetSelectedMapPiecePlacement() {
    ...
    if (!selectedMapPieceGeoreference.isPersisted && typeof window !== "undefined" && !window.confirm("Discard this unsaved placement draft?")) {
      return;
    }
  ...
  function fitSelectedMapPiece() {
    if (!selectedMapPieceGeoreference || !selectedMapPiecePlaced) {
      return;
    }
    ...
    const fitZoom = Math.min(maximumFitGpsZoom, Math.max(modernMapZoom, 17));
    requestExternalMapView(nextCenter, fitZoom, "fit_sheet", "fitSelectedMapPiece");
    commitGeographicMapSettings({ center: nextCenter, zoom: fitZoom, editMode: geoEditMode, globalHistoricalOpacity: 1 }, false);
  }
  ...
  async function saveSelectedMapPiecePlacement(): Promise<PlacementSaveResult> {
    ...
    const capturedCanonicalStatus = deriveCanonicalMapPiecePlacementStatus({ placement: capturedPlacement });
    const capturedHasGeographicFootprint = capturedCanonicalStatus === "draft" || capturedCanonicalStatus === "placed" || capturedCanonicalStatus === "reviewed";
    ...
    const normalizedSavedPlacement = normalizeSanbornMapPieceGeoreference({ ...payload.placement, pieceId: capturedPieceId, atlasPageId: capturedPiece.atlasPageId, isPersisted: true });
    const savedPlacement = deriveCanonicalMapPiecePlacementStatus({ placement: normalizedSavedPlacement }) === "placed"
      ? normalizeSanbornMapPieceGeoreference({ ...normalizedSavedPlacement, placementStatus: "placed", isPersisted: true })
      : normalizedSavedPlacement;

    const persistenceExpected = deriveCanonicalMapPiecePlacementStatus({ placement: placementForSave }) === "placed"
      ? normalizeSanbornMapPieceGeoreference({ ...placementForSave, placementStatus: "placed" })
      : placementForSave;
    if (!piecePlacementMatchesForPersistence(persistenceExpected, savedPlacement)) {
      setSaveStatus("error");
  ...
  function cancelSelectedMapPiecePlacement() {
    setPiecePlacementAnchorId("");
    setPlacementAnchorAssetId("");
    setSaveStatus("idle");
    setSaveMessage("");
  ...
  function markSelectedPlacementReviewed() {
    if (!selectedMapPiece) return;
    commitMapPieceGeoreference(selectedMapPiece.pieceId, { placementStatus: "reviewed", reviewerIdentity: "workspace reviewer", reviewedAt: new Date().toISOString() });
    setSaveMessage("Placement marked reviewed. Save placement to persist the review.");
  }
  ...
  const mapSheetLayers = showReferenceSheetAlignment && hasPlacedHistoricalSheets
    ...
    : [];
  const selectedMapPieceBounds = selectedMapPieceGeoreference && ["placed", "reviewed"].includes(deriveCanonicalMapPiecePlacementStatus({ placement: selectedMapPieceGeoreference }))
  ...
  const selectedMapPieceCanonicalStatus = deriveCanonicalMapPiecePlacementStatus({ placement: selectedMapPieceGeoreference });
  const selectedMapPieceHasGeographicFootprint = selectedMapPieceCanonicalStatus === "draft" || selectedMapPieceCanonicalStatus === "placed" || selectedMapPieceCanonicalStatus === "reviewed";
  const selectedMapPiecePlaced = selectedMapPieceCanonicalStatus === "placed" || selectedMapPieceCanonicalStatus === "reviewed";
    ...
    if (atlasWorkflowStep === "gps_alignment") {
      return (
        <MapPlacementInspector
          allMapPieceBounds={Boolean(allMapPieceBounds)}
          atlasReadOnly={atlasReadOnly}
          geoEditMode={geoEditMode}
          hasSelectedSheetGeoreference={Boolean(selectedSheetGeoreference)}
          onCancelPlacement={cancelSelectedMapPiecePlacement}
          onCenterTown={() => centerGpsOnActiveTown()}
          onConfirmUnableToPlace={markSelectedUnableToPlace}
          onDismissError={() => { setSaveStatus("idle"); setSaveMessage(""); }}
          onEditPlacement={() => { setPiecePlacementAnchorId(""); setGeoEditMode("edit_historical_sheets"); commitGeographicMapSettings({ editMode: "edit_historical_sheets", globalHistoricalOpacity: 1 }, false); setSaveStatus("idle"); setSaveMessage(""); }}
          onFitAll={fitAllPlacedMapPieces}
          onFitSelected={fitSelectedMapPiece}
          onFitSheet={fitSelectedSheet}
          onHide={() => setRightPanelCollapsed(true)}
          onMarkReviewed={markSelectedPlacementReviewed}
          onNextReview={() => movePlacementQueue("next_review")}
          onNextUnplaced={() => movePlacementQueue("next_unplaced")}
          onPreviousUnplaced={() => movePlacementQueue("previous_unplaced")}
          onPlaceSheet={() => selectedAssetId && addSheetToMap(selectedAssetId, mapCenter)}
          onReloadPlacement={() => void reloadSelectedMapPiecePlacement()}
          ...
          onSaveSheet={() => void saveSheetGeoreferences("manual", selectedAssetId)}
          ...
          onSetUnableToPlaceReason={setUnableToPlaceReason}
          onStartPlacement={startSelectedMapPiecePlacement}
          onToggleLocked={() => selectedMapPiece && selectedMapPieceGeoreference && commitMapPieceGeoreference(selectedMapPiece.pieceId, { isLocked: !selectedMapPieceGeoreference.isLocked })}
          onToggleOverlayMode={() => setOverlayRenderMode((mode) => mode === "projective" ? "rectangular" : "projective")}
          onTogglePlainMap={() => setPlainMapTestMode((value) => !value)}
          onToggleVisible={() => selectedMapPiece && selectedMapPieceGeoreference && commitMapPieceGeoreference(selectedMapPiece.pieceId, { isVisible: !selectedMapPieceGeoreference.isVisible })}
          overlayRenderMode={overlayRenderMode}
          pieceDisplayScope={pieceDisplayScope}
          plainMapTestMode={plainMapTestMode}
          placement={selectedMapPieceGeoreference}
          queue={mapPiecePlacementQueue}
          ...
          selectedMapPieceRotation={selectedMapPieceRotation}
          selectedMapPieceVisible={selectedMapPieceGeoreference?.isVisible !== false}
          selectedPage={selectedAtlasPage}
          selectedPageAssetName={selectedAtlasPageAsset?.originalFilename ?? null}
          selectedPageSourceRegionLabel={selectedPageSourceRegions.find((region) => region.linkedAtlasPageId === selectedAtlasPage?.pageId)?.regionLabel ?? null}
          selectedPageSupportsMapPlacement={selectedPageSupportsMapPlacement}
          selectedPageToolBlockMessage={selectedPageToolBlockMessage}
      ...
      <>
        <section className="sanborn-placement-queue" aria-label="Geographic object placement queue">
          ...
          <div className="sanborn-station-actions">
            ...
            <button className="sanborn-button" disabled={!mapPiecePlacementQueue.items.some((item) => item.status === "placed")} onClick={() => movePlacementQueue("next_review")} type="button">Next review-needed object</button>
          </div>
      ...
      <div className={`sanborn-atlas-workflow sanborn-atlas-workflow--stations${atlasWorkflowStep === "page_classification" ? " is-source-record" : ""}${leftPanelCollapsed ? " is-left-collapsed" : ""}${rightPanelCollapsed ? " is-right-collapsed" : ""}`}>
        ...
        {rightPanelCollapsed ? null : (
          <aside className="sanborn-station-inspector" id="sanborn-station-inspector" aria-label={`${activeStationLabel} inspector`}>
            {atlasWorkflowStep === "gps_alignment" ? renderInspectorBody() : <>
