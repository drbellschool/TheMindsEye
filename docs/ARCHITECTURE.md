...
- Sanborn editions are explicit saved atlas records; towns must not show speculative or automatically seeded years, and uploads are scoped to the active saved edition;
- edition/page management supports additive archive metadata, empty-edition deletion, scoped page moves between editions in the same town, image replacement with old-image preservation on failure, and hard-delete blocking when reconstruction dependencies exist;
- Sanborn atlas pages carry a canonical broad page type, printed reference, display title, classification notes, and explicit primary Town Index flag; Source Record is the authoritative place to classify pages and mark functional source regions;
- functional source regions identify areas inside a page, such as town coverage diagrams, sheet coverage regions, printed indexes, geographic map content, inset maps, and legends; geographic source regions can make Map Pieces available on an otherwise mixed/index page without treating the whole page as a normal Sanborn sheet;
- cover, street-index, legend, advertisement, other, and unknown pages are metadata/provenance pages unless functional regions make specific work available; existing pieces on pages without geographic page type or geographic source region are treated as conflicts and are not automatically deleted;
- Town Index is the edition mission map: it reuses durable `sheet_coverage_region` source polygons to link index-image areas to non-sequential sheets/pages, and a right-side inspector handles linkage/status editing without moving forms below the center workspace;
- Map Pieces uses a sticky source-image editing toolbar and viewport-only zoom/pan controls. Polygon geometry remains normalized to the original source image and is not saved in screen coordinates;
