...
- Konva powers the local preparation workspace for rough sheet organization, adjacency review, duplicate/missing sheet checks, and initial relative placement;
...
- Georeferencing mode is the authoritative final stitching workspace;
- each Sanborn sheet remains an independent geographic layer over the modern basemap;
- sheet-level georeferencing persists four corner coordinates, center coordinates, geographic spans, rotation, non-uniform scale, affine skew, flips, opacity, layer order, visibility, lock state, status, and review classification;
...
- current browser rendering supports real affine transforms for sheet layers; full perspective warping, GeoTIFF export, and GDAL processing remain deferred;
