...
- Konva powers the client stitching workspace for drag, scale, rotation, opacity, visibility, locking, and layer ordering;
- saved layouts persist transform and viewport data, not screenshots;
...
- georeferencing persists geographic bounds, four independent corner coordinates, control points, affine transform metadata, residuals, status, and notes;
- current overlay rendering is a rectangular preview unless and until a full warp renderer or GDAL export service is added;
