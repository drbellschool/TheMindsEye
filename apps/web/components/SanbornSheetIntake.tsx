...
function getNextSheetNumber(intake: SanbornIntakeState, localItems: LocalSanbornSheet[]): string {
  ...
  for (let sheetNumber = 1; sheetNumber <= expected; sheetNumber += 1) {
    if (!used.has(sheetNumber)) {
      return String(sheetNumber);
    }
  }
...
export function SanbornSheetIntake({ intake }: { intake: SanbornIntakeState }) {
  ...
  async function addFiles(files: FileList | File[]) {
    ...
    for (const file of selectedFiles) {
      ...
      const validation = validateSanbornFileInput({
        ...
        mimeType: file.type,
        byteSize: file.size,
        maxBytes: intake.maxUploadBytes,
      });
  ...
  function getItemWarnings(item: LocalSanbornSheet): string[] {
    ...
    if (item.validationError) {
      warnings.push(item.validationError);
    }

    if (!normalizedSheetNumber && item.status !== "unsupported") {
      warnings.push("Assign a positive sheet number before saving.");
    }

    if (normalizedSheetNumber && duplicateSheetNumbers.includes(normalizedSheetNumber)) {
      warnings.push(`Duplicate sheet number ${normalizedSheetNumber}.`);
    }
  ...
  function canUpload(item: LocalSanbornSheet): boolean {
    ...
    return intake.mode === "write_enabled" && Boolean(intakeToken.trim()) && item.status !== "uploading" && item.status !== "saved" && warnings.length === 0;
  }

  async function uploadItem(localId: string) {
    const item = localItems.find((candidate) => candidate.localId === localId);

    if (!item || !canUpload(item)) {
      return;
    }
    ...
    try {
      const response = await fetch("/api/community/sanborn-sheets", {
        method: "POST",
        headers: {
          "x-sanborn-intake-token": intakeToken.trim(),
        },
        body: formData,
      });
      ...
      if (!response.ok || !payload.ok) {
        updateItem(localId, {
          status: "failed",
          serverMessage: payload.ok ? "Sanborn upload failed." : payload.message,
        });
        return;
      }
      ...
      updateItem(localId, {
        status: "failed",
        serverMessage: "Sanborn upload failed before the server returned a response.",
      });
    }
  }
  ...
  return (
    <Panel
      ...
      subtitle="Image intake only. Stitching, georeferencing, OCR, and AI map interpretation remain out of scope."
      tone="blueprint"
      className="sanborn-intake"
      action={
        <span className={`tag state-${intake.mode === "write_enabled" ? "ready" : "guarded"}`}>
          {intake.mode === "write_enabled" ? "Storage writes enabled" : "Demo/read-only mode"}
        </span>
      }
    >
      <div className="sanborn-intake__summary">
        <div className="sanborn-intake__stat">
          <span>Town package</span>
          <strong>{intake.townPackage.packageId}</strong>
          <small>
            {intake.townPackage.name} | {intake.townPackage.region}
          </small>
        </div>
        <div className="sanborn-intake__stat">
          <span>Active map year</span>
          <strong>{intake.activeMapYear ?? "unknown"}</strong>
          <small>Year remains town-package metadata.</small>
        </div>
        <div className="sanborn-intake__stat">
          <span>Uploaded sheets</span>
          <strong>
            {intake.uploadedSheetCount} / {intake.expectedSheetCount}
          </strong>
          <small>Bucket: {intake.bucketName}</small>
        </div>
        <div className="sanborn-intake__stat">
          <span>Missing sheet warning</span>
          <strong>{renderSheetRange(missingSheetNumbers)}</strong>
          <small>Expected numbering is derived from map layers.</small>
        </div>
      </div>
      ...
      >
        ...
        <strong>
          {acceptedDescription} files up to {formatBytes(intake.maxUploadBytes)}
        </strong>
        <p className="small-muted">
          {intake.mode === "read_only"
            ? "Uploads are disabled until Supabase Storage and server-only intake configuration are available."
            : "Select multiple original sheet scans, review metadata, then save each image to Supabase Storage."}
        </p>
        <button className="sanborn-button" disabled={intake.mode === "read_only"} onClick={() => inputRef.current?.click()} type="button">
          Select Sanborn images
        </button>
      </div>

      {intake.mode === "write_enabled" ? (
        <div className="sanborn-token-row">
          <label>
            Intake token
            <input
              autoComplete="off"
              onChange={(event) => setIntakeToken(event.target.value)}
              placeholder="Temporary server-side upload token"
              type="password"
              value={intakeToken}
            />
          </label>
          <button className="sanborn-button sanborn-button--primary" disabled={!localItems.some((item) => canUpload(item))} onClick={() => void uploadReadyItems()} type="button">
            Save all ready sheets
          </button>
        </div>
      ) : null}

      {localItems.length === 0 ? (
        <div className="sanborn-empty-state">
          <strong>No Sanborn sheet images selected yet.</strong>
          <p className="small-muted">The intake queue will show thumbnails, dimensions, checksums, sheet numbers, source metadata, and review state before save.</p>
        </div>
      ) : (
          ...
          {localItems.map((item) => {
            ...
            return (
              <article className="sanborn-file-card" key={item.localId}>
                <div className="sanborn-file-card__preview">
                  <img alt={`Preview of ${item.originalFilename}`} src={item.previewUrl} />
                </div>
                <div className="sanborn-file-card__body">
                  <div className="record-list__header">
                    <div>
                      <p className="record-list__eyebrow">Original filename</p>
                      <strong className="record-list__title">{item.originalFilename}</strong>
                      <p className="small-muted">
                        {formatBytes(item.byteSize)} | {item.mimeType} | {item.width && item.height ? `${item.width} x ${item.height}` : "dimensions unavailable"}
                      </p>
                    </div>
                  ...
                  <div className="sanborn-file-card__metadata">
                    <label>
                      Sheet number
                      <input inputMode="numeric" onChange={(event) => updateItem(item.localId, { sheetNumber: event.target.value })} value={item.sheetNumber} />
                    </label>
                    <label>
                      Source record association
                      <select onChange={(event) => handleSourceChange(item.localId, event.target.value)} value={item.sourceRecordId}>
                        <option value="">Source unavailable / no linked source</option>
                        {intake.sourceOptions.map((sourceOption) => (
                          <option key={sourceOption.sourceRecordId} value={sourceOption.sourceRecordId}>
                            {sourceOption.sourceId} - {sourceOption.title}
                          </option>
                        ))}
                      </select>
                    </label>
                    <label>
                      Source URL
                      <input onChange={(event) => updateItem(item.localId, { sourceUrl: event.target.value })} placeholder="Unknown or unavailable" value={item.sourceUrl} />
                    </label>
                    <label>
                      Archive name
                      <input onChange={(event) => updateItem(item.localId, { archiveName: event.target.value })} placeholder="Unknown or unavailable" value={item.archiveName} />
                    </label>
                    <label>
                      Rights note
                      <input onChange={(event) => updateItem(item.localId, { rightsNote: event.target.value })} placeholder="Unknown or unavailable" value={item.rightsNote} />
                    </label>
                    <label>
                      Evidence classification
                      <select disabled value={sanbornDefaultEvidenceClassification}>
                        <option value={sanbornDefaultEvidenceClassification}>{sanbornDefaultEvidenceClassification}</option>
                      </select>
                    </label>
                    <label>
                      Review status
                      <select disabled value={sanbornDefaultReviewStatus}>
                        <option value={sanbornDefaultReviewStatus}>{sanbornDefaultReviewStatus}</option>
                      </select>
                    </label>
                    <label className="sanborn-file-card__wide">
                      Intake notes
                      <textarea onChange={(event) => updateItem(item.localId, { intakeNotes: event.target.value })} placeholder="Describe scan quality, source context, or sheet-number uncertainty." value={item.intakeNotes} />
                    </label>
                  </div>

                  <div className="sanborn-checksum">
                    <span>SHA-256 checksum</span>
                    <code>{item.checksum ?? "Unavailable in this browser until server save."}</code>
                  </div>

                  {warnings.length > 0 ? (
                    <div className="sanborn-warning-list">
                  ...
                  <button className="sanborn-button sanborn-button--primary" disabled={!canUpload(item)} onClick={() => void uploadItem(item.localId)} type="button">
                    {item.status === "uploading" ? "Saving..." : item.status === "saved" ? "Saved" : "Save original sheet"}
                  </button>
                </div>
              </article>
            );
      ...
      <div className="sanborn-result-grid">
        <div className="small-card">
          <p className="muted">Successful uploads this session</p>
          <strong>{savedItems.length}</strong>
          <p className="small-muted">{savedItems.length > 0 ? savedItems.map((item) => item.originalFilename).join(", ") : "No successful uploads yet."}</p>
        </div>
        <div className="small-card">
          <p className="muted">Failed or rejected uploads</p>
          <strong>{failedItems.length}</strong>
          <p className="small-muted">{failedItems.length > 0 ? failedItems.map((item) => item.originalFilename).join(", ") : "No failed uploads yet."}</p>
        </div>
      </div>

      <div className="sanborn-existing-list">
        <h3>Stored intake records</h3>
        {intake.existingAssets.length === 0 ? (
          <p className="small-muted">No Sanborn sheet assets are stored yet.</p>
        ) : (
          intake.existingAssets.map((asset) => (
            <div className="small-card" key={asset.assetId}>
              <p className="muted">Sheet {asset.sheetNumber ?? "unknown"}</p>
              <strong>{asset.originalFilename}</strong>
              <p className="small-muted">
                {formatBytes(asset.byteSize)} | {asset.width && asset.height ? `${asset.width} x ${asset.height}` : "dimensions unavailable"} | {asset.reviewStatus}
              </p>
              <p className="small-muted">Source: {asset.sourceId ?? "Source unavailable"}</p>
              <p className="small-muted">Checksum: {asset.checksum ?? "unavailable"}</p>
            </div>
          ))
        )}
      </div>
