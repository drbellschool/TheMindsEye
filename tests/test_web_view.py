...
class WebViewTests(unittest.TestCase):
    def test_view_model_uses_loaded_package_and_mission_packet(self):
        ...
        self.assertEqual(model["community_dashboard"]["dashboard_title"], "Texarkana Community Verification Console")
        self.assertEqual(model["community_dashboard"]["year_gate"]["start_year"], 1875)
        self.assertEqual(model["community_dashboard"]["community_review"]["source_issue_count"], 1)
        self.assertEqual(model["community_dashboard"]["entity_review_panels"]["people"][0]["display_name"], "A. S. Blythe")
        self.assertIn("Exchange", model["community_dashboard"]["entity_review_panels"]["businesses"][0]["display_name"])
        self.assertEqual(model["map_auditor"]["dashboard_title"], "Texarkana Community Map Auditor")
        self.assertEqual(model["map_auditor"]["selected_sheet"]["sheet_label"], "003")
        self.assertEqual(model["map_auditor"]["selected_building"]["building_id"], "building_texarkana_1885_003")
        self.assertEqual(
            model["map_auditor"]["provenance_trail"]["source_issue"]["publication_title"],
            "Daily Texarkana Democrat",
        )
        self.assertEqual(model["people_auditor"]["dashboard_title"], "Texarkana People Auditor")
        self.assertEqual(model["people_auditor"]["selected_issue"]["publication_title"], "Daily Texarkana Democrat")
        self.assertEqual(model["people_auditor"]["selected_person"]["display_name"], "A. S. Blythe")
        self.assertEqual(
            model["people_auditor"]["selected_business"]["display_name"],
            "Exchange (H. T. Huey Proprietor)",
        )
        self.assertEqual(model["campaign"]["mastery_checkpoint_count"], 16)
        self.assertEqual(model["campaign"]["year_gate"]["end_year"], 1895)
        self.assertEqual(model["community_review"]["source_issues"][0]["publication_title"], "Daily Texarkana Democrat")
    ...
    def test_rendered_page_contains_read_only_town_data(self):
        ...
        self.assertIn("Base Map Layer", html)
        self.assertIn("Manifest control points", html)
        self.assertIn("Workspace control points", html)
        self.assertIn("Local alignment", html)
        self.assertIn("Missing control points", html)
        ...
        self.assertIn("review-editor", html)
        self.assertIn("/api/review-action", html)
        self.assertIn("Year Gate", html)
        self.assertIn("Review Status Overview", html)
        self.assertIn("Scope Ladder", html)
        self.assertIn("Primary Routes", html)
        self.assertIn("Evidence Inspector", html)
        self.assertIn("Unresolved Summary", html)
        self.assertIn("Quick Actions", html)
        self.assertIn("Diagnostics &amp; Safeguards", html)
        self.assertIn("Community Reviewer", html)
        self.assertIn("Local build snapshot", html)
        self.assertIn("Community Map Auditor", html)
        self.assertIn("Back to Community", html)
        self.assertIn("Open Building Auditor", html)
        self.assertIn("People Auditor", html)
        self.assertIn("Source Issue Browser", html)
        self.assertIn("People Review Workspace", html)
        self.assertIn("Business Review Workspace", html)
        self.assertIn("County Manager", html)
        self.assertIn("State Manager", html)
        self.assertIn("People / Businesses", html)
        self.assertIn("Daily Texarkana Democrat", html)
        self.assertIn("A. S. Blythe", html)
        self.assertIn("H. T. Huey", html)
        self.assertIn("Campaign Framework", html)
        self.assertIn("16 checkpoints", html)
        self.assertIn("1875", html)
        self.assertIn("1895", html)
